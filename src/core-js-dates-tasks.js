...
function getNextFriday(date) {
  ...
  if (day === 5) {
    ...
    daysUntilNextFriday = (5 - day + 7) % 7;
  }

  const nextFriday = new Date(
    date.getTime() + daysUntilNextFriday * 24 * 60 * 60 * 1000
  );
...
function isDateInPeriod(date, period) {
  ...
  const startObject = new Date(period.start);
  const endObject = new Date(period.end);
...
function getWeekNumberByDate(date) {
  ...
  const daysDifference = Math.floor(
    (copyDate - firstDayOfYear) / (1000 * 60 * 60 * 24)
  );

  const weekNumber = Math.floor(daysDifference / 7) + 1;

  if (weekNumber === 1 && firstDayOfYear.getDay() !== 1) {
    const firstMondayOfYear = new Date(firstDayOfYear);
    firstMondayOfYear.setDate(
      firstMondayOfYear.getDate() + ((7 - firstMondayOfYear.getDay()) % 7)
    );
    const daysDifferenceAdjusted = Math.floor(
      (copyDate - firstMondayOfYear) / (1000 * 60 * 60 * 24)
    );
    if (daysDifferenceAdjusted < 0)
      return getWeekNumberByDate(new Date(copyDate.getFullYear() - 1, 11, 31));
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= new Date(period.end.split('-').reverse().join('-'))) {
    if (workDayCount < countWorkDays) {
      schedule.push(
        `${startDate.getDate().toString().padStart(2, '0')}-${(startDate.getMonth() + 1).toString().padStart(2, '0')}-${startDate.getFullYear()}`
      );
      workDayCount += 1;
    } else if (offDayCount < countOffDays) {
      offDayCount += 1;
    }
    if (workDayCount === countWorkDays && offDayCount === countOffDays) {
      workDayCount = 0;
      offDayCount = 0;
    }
